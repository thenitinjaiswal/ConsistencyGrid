 ...
 */
export default function ThemeSelector({ activeTheme, onChange, isFreeUser = false }) {
    const [showUpgradePrompt, setShowUpgradePrompt] = useState(false);

    const handleThemeClick = (themeId, isPro) => {
        if (isFreeUser && isPro) {
    ...
    return (
        <div className="space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                {THEMES.map((theme) => {
                    const isPro = !['minimal-dark', 'monochrome'].includes(theme.id);
                    const isLocked = isFreeUser && isPro;

                    return (
                        <button
                            key={theme.id}
                            onClick={() => handleThemeClick(theme.id, isPro)}
                            title={isLocked ? "Unlock with Pro" : `Select ${theme.name}`}
                            className={`group relative flex items-center gap-3 rounded-xl border-2 p-3 sm:p-4 text-left transition-all duration-200 ${activeTheme === theme.id
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                    ...
                                    <div className="min-w-0 flex-1">
                                        <h4 className={`text-sm font-bold truncate flex items-center gap-2 ${activeTheme === theme.id ? "text-orange-700" : "text-gray-900"
                                            }`}>
                                            {theme.name}
                                            {isPro && !isLocked && (
                                                <span className="text-[10px] bg-orange-100 text-orange-600 px-1.5 py-0.5 rounded-full uppercase tracking-wider font-bold">PRO</span>
                                            )}
                                        </h4>
                            ...
                            {activeTheme === theme.id ? (
                                ...
                                <div className="absolute top-2 right-2 bg-gray-900/80 text-white rounded-full p-1.5 shadow-lg backdrop-blur-sm">
            ...
            <UpgradePrompt
                ...
                message="Upgrade to Pro to access all exclusive themes and customization options."
